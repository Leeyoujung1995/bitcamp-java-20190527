...
static Queue<String> commandQueue = new Queue();
  ...
  public static void main(String[] args) throws Exception {
   ...
   Input input = new Input(keyScan);
    ...
    BoardHandler boardHandler2 = new BoardHandler(input,new LinkedList<>());//boardHandler 어떤 인스턴스를 써줄지만 해준다.//같은 메소드로 주소값만 다르게                                                    
 ...
 private static void printCommandHistory() throws Exception{
   ...
   int count =0;
    while (!stack.empty()) {
     ...
     if(++count % 5 == 0) {
       System.out.println(":");
       if(keyScan.nextLine().equalsIgnoreCase("s"))
         break;
     }
    }
