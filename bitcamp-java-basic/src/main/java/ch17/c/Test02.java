// gernaralization을 통해 수퍼 클래스를 정의할 때 문제점
package ch17.c;
...
public class Test02 {
  ...
  public static void main(String[] args) {
    ...
    // => Sorter는 BubbleSort와 QuickSort 클래스처럼 단지 정렬을 수행하는 클래스를 만들 때 일관성 있게 하려고
    //    수퍼 클래스로서 정의한 것이다
    // => Sorter는 직접 사용하려고 만든 것이 아니다.
    
    //다음과 같이 Sorter 를 직접 사용하는 것은 아무런 의미가 없다.
    ///왜? 정렬을 수행하는 메서드인 sort()가 아무 일도 안한다.
    display(new Sorter(), values);
    
    //이런 경우, 차라리 Sorter 클래스를 추상 클래스로 만드는 것이 바람직하다.
    //추상 클래스인 경우 직접 인스턴스를 만들어 사용할 수 없기 떄문이다.
  }
