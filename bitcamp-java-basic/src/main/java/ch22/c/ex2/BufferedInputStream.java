...
public class BufferedInputStream extends InputStream {
  ...
  public int read() throws IOException {
    ...
    //배열에 들어 잇는 바이트의 값이 양수일 경우 int로 리턴하면 그 값 그대로 리턴한다
    //예)0x7f(10진수 127) 값을 리턴한다면 4바이트 int 값 0x0000007f가 리턴된다
    //문제는 첫 비트가 1로 시작하는 바이트 값을 int로 리턴하는 경우이다
    //JVM은 첫 비트가 1일 경우 음수 간주하고
    //byte 값을 int로 바꿀 때 앞의 3바이트를 모두 음수를 의미하는1로 설정한다
    //예)0x80(10000000,10진수로 128)값을 리턴한다면
    //4바이트 int값 0xffffff80이 리턴된다
    //즉 바이트 배열에 들어 잇는 값이 첫 비트가 1로 시작할 경우 이런 오류가 발생하는 것이다.
    //원래의 바이트 값을 오전히 4바이트로 바꿔 리턴하고 싶다면
    //앞의 3바이트를 무조건 0으로 만들면 된다.
    //즉 다음과 같이 4바이트로 변환된 값을 리턴하기 전에 비트 연산자 앤드를 사용하여 앞의 3바이트를 0으로 설정하라
    //예)0x80->0xfffffff80 &0x000000ff -->0x00000080
