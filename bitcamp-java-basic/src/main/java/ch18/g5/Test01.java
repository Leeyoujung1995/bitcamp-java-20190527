...
public class Test01 {
  ...
  public static void main(String[] args) {
    ...
    //기존 규격에 포함시키는 방법
    //=>디폴트 메서드를 추가하라
    //=>g2패키지의 Printer 인터페이스에 watermark() 메서드는
    //디폴트 메서드로 선언하면 된다.
    ...
    //프린터를 사용하여 출력한다
    display(p1,"안녕하세요!","bitcamp bitcamp bitcamp bitcamp");
    
    //기존 프린터 준비!
    PaperPrinter p2 = new PaperPrinter();
    FilmPrinter p3 = new FilmPrinter();
    //기존 규격(Printer)에 새규칙(watermark())을 추가하더라도
    //기존에 작성한 클래스를 그대로 활용할 수 있다
    //굳이 g4 패키지에서 한 것처럼 아답터 패턴을 사용할 필요도 없다
  
 
    //컴파일 오류!!
    display(p2,"안녕하세요!","bitcamp bitcamp bitcamp bitcamp");
    display(p3,"안녕하세요!","bitcamp bitcamp bitcamp bitcamp");
  }
