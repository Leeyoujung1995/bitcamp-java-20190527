...
//역할만 하기 떄문에 이 클래스가 직접 Printer 기능을 구현해서는 안된다.
//대신 Printer 기능은 기존 클래스에게 맡겨야 한다.
 ...
 public void print(String text) {
   //아답터는 Printer2의 규격을 따르기는 하지만 직접 Printer 역할을 하는 것이 아니기 떄문에
   //아답터는 print()라는 명령을 내리면,
   //생성자에서 받은 기존 프린터 객체를 실행시킨다.
   originalPrinter.print(text);
 }
 
 public void watermark(String title) {
   //단 Printer2에 추가된 워터마킹 기능은 아답터에서 실행된다
