// 추상 클래스를 이용하여 인터페이스를 구현하고 인터페이스의 일부 메서드를 미리 정의하면 서브 클래스가
// 인터페이스의 모든 메서드를 정의해야 하는 부담을 줄일 수 있다.
//인터페이스에 메서드가 많을 떄는 특히 이런 방식을 사용한다.
//이런 방식을 실무에서 많이 사용한다.
//즉 인터페이스를 직접 구현하기 보다는 추상 클래스를  상속받아 간접적으로 구현하는 기법을 뜻한다.
package ch18.f;
...
public abstract class AbstractCar implements CarSpec{
  ...
  //그중에서 인터페이스에 선언된 메서드의 일부를 미리 구현해주면 서브클래스를 만들기 매우 편리하다
  public void on() {
