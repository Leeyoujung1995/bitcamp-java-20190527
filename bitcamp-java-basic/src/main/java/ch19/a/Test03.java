// 중첩 클래스 사용 I:static nested class와 inner클래스 사용
package ch19.a;
...
class X{
  ...
  //컴파일 오류가 발생한 이유?
  //=>다음과 같이 스태틱 메서드에서 인스턴스 메서들를 호출하지 못하는 이유와 같다
  ...
  obj2 =x.new B();//실무에서는 이렇게 외부의 인스턴스를 가지고 inner 클래스를 생성하는 방식을 거의 사용 x 그냥 참고!
  ...
  //스태틱 멤버는 인스턴스 멤버에서도 자유롭게 사용 할 수 있다.
