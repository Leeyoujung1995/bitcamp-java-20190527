...
class Monitor4 {
  ...
  //호출된 생성자는 new 명령에서 지정한다.
  //예)new 클래스명(호출할 생성자의 전달할 파라미터 값)
  //=>다음과 같이 파라미터의 값을 주지 않으면 파라미터 값을 안받는 생성자가 호출된다.
  //예)Monitor4()
  //=>파라미터 값을 안받는 생성자(파라미터가 없는 생성자)를 기본 생성자(default constructor)라 부른다.
  //=>new 명령을 실행항 인스턴스를 생성할 때는 반드시 호출될 생성자를 지정해야 한다
  //예)new 클래스명();
  ...
  Monitor4() { 
    ...
    this.widthRes = 1920;
    this.heightRes = 1080;
    //인스턴스 초기화 블럭이 있는데 왜 성성자가 필요한가?
    //-생성자는 외부에서 초기화시킬 값을 받을 수 있다.
    //-다음 현재의 생성자는 기본 생성자로서 외부로부터 값을 받지 않기 때문에 초기화 블록이랑 다를 바가 없다
  }
...
public class Test06 {
  public static void main(String[] args) {
    // 모니터 인스턴스 생성
    //1)Monitor4 설계도에 따라 인스턴스 필드를 Heap에 생성된다.
    //2)필드의 초기화 문장을 실행한다.
    //예)int bright =10;
    //3)인스턴스 초기화 블록을 실행한다.
    //예){
