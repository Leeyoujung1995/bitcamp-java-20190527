...
public class App  implements Servlet{
  ...
  public void init(ServletConfig config) throws ServletException {
    this.config=config;
    //톰캣 서버가 이 객체를 사용하기 전에 이 객체가 작업에 필요한 자원들을 준비할 수 있도록 이메서들를 호출한다.   
    ...
    try {
      ...
      if (requestHandler != null) {
        ...
        requestHandler.method.invoke(requestHandler.bean, 
            ...
            out.println(
            "<html><body><h1>해당 명령을 처리할 커맨드를 찾을 수 없습니다.</h1></body></html>");   
  ...
  public String getServletInfo() {
    ...
    //이 메서드를 호출한다. 즉 이 메서드의 리턴값을 관리자 페이지에 출력한다
    return "수업관리 시스템";
  }
  ...
  public ServletConfig getServletConfig() {
    ...
    //리턴 값은 init() 메서드에서 파라미터 값을 잘 보관해 둬야 한다.
    return this.config;
  }
