...
public class PhotoBoardCommand  {
  ...
  public PhotoBoardCommand(PlatformTransactionManager txManager,
  ...
  public void delete(BufferedReader in ,PrintStream out) {
    ...
    out.println("데이터를 삭제하였습니다.");
      ...
      //=>DBMS 쪽 담당자(스레드)에게 임시 보관된 데이터 변경 결과를 모두 취소할 것을 명령한다.
      //예외가 발생하면 DBMS의 임시 데이터베이스에 보관된 데이터 변경 작업들을 모두 취소한다.
      try {txManager.rollback(status);}catch(Exception e2) {}
      out.println("데이터 저장에 실패했습니다!");
      System.out.println(e.getMessage());
      e.printStackTrace();
    }
  ...
  public void update(BufferedReader in ,PrintStream out) {
    ...
    try {
      ...
      try {txManager.rollback(status);}catch(Exception e1) {}
