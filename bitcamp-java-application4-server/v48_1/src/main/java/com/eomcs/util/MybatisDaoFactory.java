  ...
  public <T> T createDao(Class<T> clazz) {
    return (T)Proxy.newProxyInstance(
        ...
        (Object proxy, Method method, Object[] args) -> {
          //InvocationHandler 구현체의 람다메서드
          //자동으로 생성된 DAO 구현체를 대해 메서드를 호출하면 최종적으로 이 메서드가 호출된다.
          ...
          try(SqlSession sqlSession = sqlSessionFactory.openSession()) {
            ...
            //=>int라면 insert()/update()/delete()호출한다.
            ...
            if(retType == List.class) {
              ...
              //리턴 타입이 int나 void일 경우 insert/update/delete문을 실행한는 것이다
