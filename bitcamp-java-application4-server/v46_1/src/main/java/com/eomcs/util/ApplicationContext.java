...
public class ApplicationContext {
  ...
  private void createCommand() {
    for(Class<?> clazz : classes) {
      ...
      try {
        ...
        //준비된 ㄱ밧을 가지고 생성자를 통해 인스턴스를 생성한다
  ...
  private Object getBean(Class<?> type) {
   ...
   throw new RuntimeException("해당 타입의 빈을 찾을 수 없습니다:" + type.getName());
  }

  public Object getBean(String name){
    Object obj = objPool.get(name);
    if(obj == null)
      throw new RuntimeException("해당 이름의 빈을 찾을 수 없습니다:" + name);
    return obj;   
  }
  ...
  private void createDao() throws Exception{
    ...
    //Dao구현체 생성기 준비한다
    MybatisDaoFactory daoFactory = new MybatisDaoFactory((SqlSessionFactory)objPool.get("sqlSessionFactory"));
