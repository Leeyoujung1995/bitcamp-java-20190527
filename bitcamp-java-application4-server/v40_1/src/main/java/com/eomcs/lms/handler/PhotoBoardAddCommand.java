...
public class PhotoBoardAddCommand implements Command {
  ...
  public PhotoBoardAddCommand(ConnectionFactory conFactory,
  ...
  public void excute(BufferedReader in, PrintStream out) {
    ...
    Connection con = null;
  try {    //DAO에게 작업을 시키기 전에 커넥션 팩토리로부터 커넥션을 얻는다
      //=>그러면 커넥션 팩토리는 스레드 주머니에 커넥션을 담아 둘 것이다.
      //=>그 이후에 DAO가 커넥션 팩토리에게 커넥션을 요구하면
      //  바로 이 스레드 주머니에 들어 있는 커넥션 객체를 리턴할 것이다
      //=>따라서 이 메서드가 끝날 때 까지 DAO는 같은 커넥션을 사용할 것이더
      con = conFactory.getConnection();
      //DAO가 사용할 커넥션에 대학 AUTOCOMMIT을 false로 설정
      ...
      //=>DBMS쪽 담당자(스레드)에게 임시 보관된 데이터 변경 결과를
      //실제 테이블에 적용할 것을 명령한다.
      con.commit();
      out.println("저장하였습니다.");
